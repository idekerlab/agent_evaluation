...
class ReviewRunner:
    ...
    def next_review(self):
        ...
        for analyst_id in self.review_plan.analyst_ids:
            if len(self.review_set.attempts.get(analyst_id, [])) < 1:
                try:
                    generator = ReviewGenerator(self.db)
                    review_id = generator.generate_review(analyst_id, self.analysis_run.dataset_id, self.hypotheses_text, self.analysis_run.object_id, self.review_set.object_id)
                    self.review_set.review_ids.append(review_id)
                    self.review_set.attempts[analyst_id].append('success')
                    self.review_set.update()
                    return f"New review generated by analyst {analyst_id}."
                except Exception as e:
                    self.review_set.attempts[analyst_id].append('failed')
                    self.review_set.update()
                    return f"Failed to generate review for analyst {analyst_id}: {str(e)}"
