...
from ae_object import AE_Object
from abc import ABC, abstractmethod
import logging

class LLM(AE_Object, ABC):
...
class OpenAI_LLM(LLM):
    def __init__(self, db, model_name, temperature=0.5, max_tokens=2048, seed=None, persist=False, get=False):
        """
        Initializes a new OpenAI LLM instance with specific configurations.

        :param db: Database instance for persistence.
        :param model_name: The name of the OpenAI model to query.
        ...
        :param get: Whether to attempt to retrieve the model instance from the database.
        """
        super().__init__(db, model_name, temperature, max_tokens, seed, persist, get)
    ...
    def query(self, context="you are a helpful assistant", prompt="What number is the meaning of life?"):
        ...
        :param context: The context to use when querying the model.
        :param prompt: The prompt to use when querying the model.
        :return: A tuple containing the model's response to the prompt, system fingerprint, and tokens used.
        """
        ...
        openai.api_key = key
        backoff_time = 10  # Start backoff time at 10 seconds
        retries = 0
